	...
	class Perizinan extends MY_Controller {
		...
		public function load() {
			...
			foreach($rResult->result_array() as $aRow) {
				...
				$final[] = $row;
                        ...
                        $perizinan = array("Izin Mendirikan Bangunan (IMB)", "Surat Izin Usaha Perdagangan (SIUP)", "Tanda Daftar Perusahaan (TDP)","Izin Peruntukan Penggunaan Lahan (IPPL)","Izin Reklame","Izin Praktek Dokter","Surat Izin Usaha Kepariwisataan");

			$items = array();
			$no = 1;
			foreach($final as $row) {

				$items[] = array($no,$row['kode_klasifikasi'],'' . $row['kurun_waktu_awal'] . ' - ' . $row['kurun_waktu_akhir'],'' . $perizinan[array_rand($perizinan)] . '','' . $row['uraian'] . '','' . $row['tingkat_perkembangan'] . '','' . $row['jumlah_berkas'] . '','<button class="btn btn-info" onclick="formUpload(' . $row['id_arsip'] . ');">  <i class="clip-folder-upload"></i> </button>','<a href="#" role="button" onclick="formDetilPenerimaan(' . $row['id_arsip'] . ');" class="btn btn-default" data-toggle="modal"> Detail </a>');
				$no++;
			}
		...
		private function successImport() {
			echo '<script>
                ...
                window.location.replace("' . base_url() . 'pemindahan/entry");
                  </script>';
		}
